...
(deftest winner?-test
  (testing "Detect a winner on a row"
    (is (winner? (vec (repeat 9 "X"))))
    (is (winner? (vec (concat (repeat 3 "X") (repeat 6 \space))))))
  (testing "Detect a winner on a diagonal"
    (is (winner? [\space \space "X" \space "X" \space "X" \space \space])))
  (testing "Detect a winner on a column"
    (is (winner? [\space \space "X" \space \space "X" \space \space "X"])))
